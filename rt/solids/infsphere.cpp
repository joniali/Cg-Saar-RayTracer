#include <rt\solids\infsphere.h>

namespace rt {
	InfSphere::InfSphere(const Point& center, float radius, CoordMapper* texMapper, Material* material) :Solid(texMapper, material)
	{
		scenter = center;
		sradius = radius;
	...
	Intersection InfSphere::intersect(const Ray& ray, float previousBestDistance) const{
	...
	Point InfSphere::sample() const
	{

		return Point(0, 0, 0);
	}
	float InfSphere::getArea() const
	{
		return 4 * pi*sradius*sradius;
	}
	BBox InfSphere::getBounds() const
	{
